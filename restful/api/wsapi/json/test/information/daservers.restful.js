...
var base_url = base+"/v2/json/daservers";
var auth_url = base+"/v2/json/auth"

var accessKey,
 	daserver_1_name = "daserver_name_1_test"+Date.now().valueOf(),
 	daserver_1_id;
...
describe('create daserver', function () {
	it("should daserver without error", function(done){
		var url = base_url+"?accesskey="+accessKey;
		var flow = {
			main: [{ post: url, rejectUnauthorized: false, json:{"name":daserver_1_name},
		    	afterHooks: [ 
		    	function (all) {
		    		daserver_1_id = all.body.ret;
...
describe('get  daserver ', function () {
	it("should get daserver without error", function(done){
		var url = base_url+"/"+daserver_1_id+"?accesskey="+accessKey;
		var flow = {
...
describe('get  daserver  by  name ', function () {
	it("should get daserver  by name without error", function(done){
		var url = base_url+"?name="+daserver_1_name+"&accesskey="+accessKey;
		var flow = {
			main: [{ get: url, rejectUnauthorized: false,
		    	afterHooks: [ 
		    	function (all) {
...
describe('modify daserver ', function () {
	it("should modify daserver without error", function(done){
		var url = base_url+"/"+daserver_1_id+"?accesskey="+accessKey;
		var flow = {
			main: [{ put: url, rejectUnauthorized: false,json:{"desc":"test for modify daserver"},
		    	afterHooks: [ 
		    	function (all) {
		    		return verifyData(all);
		    	}
...
describe('delete daserver ', function () {
	it("should delete daserver without error", function(done){//debugger
		var url = base_url+"/"+daserver_1_id+"?accesskey="+accessKey;
		var flow = {
