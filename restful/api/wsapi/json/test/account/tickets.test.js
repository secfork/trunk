...
describe("get tickets of system", function(){
	it("should without error", function(done){
		request.get({
			...
			assert.equal(null, err);
			assert.equal(null, verifyData(body).err);
			assert.equal(2, verifyData(body).ret.length);
			//console.dir(verifyData(body).ret);
			done();
		});
	});
...
// set ticket_2 of system
describe("delete ticket_2 of system", function(){
	it("should without error", function(done) {
		var fields = {
			"ticket" : ticket_2,
			"expire" : 50,
			"privilege" : ["READ_DATA", "WRITE_DATA", "SYSTEM_CONTROL"]
		};
