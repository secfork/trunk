/*
 *	@name : get history read raw by system model
 *	@kind : datasource prototype
 *	@description : getSystemHistoryReadRaw ProtoType
 *  @return : [ { } ]
 */
...
var module = angular.module("getSystemHistoryReadRaw" , []);

module.service('$getSystemHistoryReadRaw' , function($rootScope , $_getAccessKey , $http , $datasource_factory , $service_progress , $unique_id , $console , $filter){
	...
	this.ds_prototype = function(){
		...
		this.options = [
			...
			{"oname" : "start" , "value" : null , "type" : "date"},
			{"oname" : "end" , "value" : null , "type" : "date"},
			{"oname" : "limit" , "value" : null , "type" : "number"},
			{"oname" : "timestamp" , "value" : null , "type" : "string"},
			{"oname" : "bound" , "value" : null , "type" : "string"}
		];
		this.push_data;
		this.event_id;
		this.data_type = 1;
		this.directive = "<div get-system-history-read-raw id='datasource_directive'></div>";
	}
	...
	this.ds_prototype.prototype.getMeta = function(callback){
		...
		var getField = function(data){
			...
			$console.log(meta , 'getSystemHistory meta format - getSystemHistory.js');
			callback(meta);
	...
	var req = function(uuid , _this , callback){
		...
		$_getAccessKey.getAck(function(ack){
			url = "/v2/json/systems/"+ uuid +"/log/readraw?" + ack ;
			...
			url += "&timestamp=" + new Date(_this.options[4].value).getTime();

			url += "&bound=" + _this.options[5].value;
	...
	var dataParse = function(data , _this){
		...
		if(tags.length == 0){
			$console.log(data , "system live no tag , getSystemHistoryReadRAW.js");
			return [];
		}else{
			var list = [];
			var limit = 1000;
			...
			for(var i=0;i<limit;i++)
			{
				...
				$.each(tags , function(idx , val){
					$.each(data , function(cidx , cval){
						json[val] = data[cidx][i];
					})
				})
				list.push(json);
			}

			$console.log(list , "system history read raw , getSystemHistoryReadRaw.js")
			return list;
		}
