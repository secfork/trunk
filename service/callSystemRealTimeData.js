/*
 *	@name : call system device data
 *	@kind : datasource prototype
 *	@description : callSystemRealTimeData ProtoType
 *  @return : object
 */
...
var module = angular.module("callSystemRealTimeData" , []);

module.service('$callSystemRealTimeData' , function($rootScope , $_getAccessKey , $http , $datasource_factory , $unique_id , $service_progress){
	...
	var req = function(uuid , _this , callback){
		var data = {"type" : "CALL_REALTIME"};
		$_getAccessKey.getAck(function(ack){
			var url = "/v2/json/systems/"+ uuid +"/control/poll?" + ack ;
			$service_progress.start();
			$http.post(url , data).success(function( data , state ){
