/*
 *	@name : set data to system
 *	@kind : datasource prototype
 *	@description : getSystemWrite ProtoType
 *  @return : null
 */
...
var module = angular.module("getSystemWrite" , []);

module.service('$getSystemWrite' , function($rootScope , $_getAccessKey , $http , $datasource_factory , $service_progress , $unique_id){
	...
	this.ds_prototype = function(){
		...
		this.index = 0;
		this.options = [
			{"oname" : "tagname" , "value" : [] , "type" : "array"},
		];
		this.push_data;
		this.event_id;
		this.data_type = 0;
	...
	var req = function(uuid , _this , callback){
		...
		var type = _this.parameters[0].value;
		$_getAccessKey.getAck(function(ack){
			var url = "/v2/json/systems/"+ uuid +"/control/write?" + ack ;
			$service_progress.start();
			$http.post(url , _this.push_data).success(function( data , state ){
				$service_progress.stop();
				$rootScope.$broadcast(_this.event_id , data);
				callback(data)
			});
		});
